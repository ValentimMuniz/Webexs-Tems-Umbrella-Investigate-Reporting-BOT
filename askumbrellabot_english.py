...
def help_me():
    return "Sure! I can help. Below are the commands that I understand:<br/>" \
           ...
           "`domain:` - Enter the domain to query - Example 'domain: internetbadguys.com'<br/> or domain internetbadguys.com" \
           "`toplist XX` - I will display the top Umbrella Popularity Domain List - Global list - Ex: toplist 25'<br/>" \
           "`threats` - I will display the top Umbrella Threats in 1 month<br/>" \
           "`total` - I will display the total Umbrella Requests in 1 month<br/>" \
           "`summary or summ` - I will display Umbrella Summary Requests in 1 month<br/>" 
...
def totalrequests(totalreq):
    return "Cisco Umbrella Total Requests: <br><b>{}</b>".format(totalreq)

def summary(sum):
    return "Cisco Umbrella Summary: <b>{}</b>".format(sum)
...
def umbrella_get(varDOMAIN, roomType, room_title, today, timestamp, personEmail):
        ...
        if var_RESPONSE_CATEGORIES.status_code == 403 or var_RESPONSE_CATEGORIES.status_code == 500:
            return "Looks like the provided Investigate token is not correct. Please review the config file!"
            ...
            if check_room_id == logs_roomId:
                ...
                if mensagem in expected_messages and expected_messages[mensagem] == "help":
                    ...
                    if len(mensagem) > 7:
                        ...
                        msg = 'Missing parameters! Usage - domain: internetbadguys.com'
                    ...
                    if len(mensagem) > 7:
                        ...
                        if CheckInt(top) == True:
                            ...
                            msg = 'Type numbers only'
                    else:
                        msg = 'Missing parameters! Usage - toplist XX'
                    ...
                    if len(mensagem) > 3:
                        ...
                        msg = 'Missing parameters! Usage - top XX'
                elif mensagem == "threats":
                    access_token = get_access_token()

                    if code_access_token == 403 or code_access_token == 401:
                        msg = "Looks like the provided Management Key or Managemente Secret is not correct. Please review the config file!"
